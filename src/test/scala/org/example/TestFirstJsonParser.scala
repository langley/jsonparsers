...
class TestFirstJsonParser extends FlatSpec {
  val fjp = new FirstJsonParser
  
  "FirstJsonParser" should "parse a.json w/out failure" in {
	val reader = Source.fromURL(getClass.getResource("/a.json")).reader
	val result = fjp.parseAll(fjp.obj, reader)
	result should be ('successful)
  }
  
  "FirstJsonParser" should "succeed because this parser just looks for json" in {
  ...
  "FirstJsonParser" should "fail a parse of b.json because it's not an addressBook" in {
  ...
  "FirstJsonParser" should "fail a parse of bFail.json because it's not even json" in {
